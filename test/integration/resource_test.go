...
func TestResourceFromGRPC(t *testing.T) {
	...
	if err := testutil.GatherAndCompare(prometheus.DefaultGatherer,
		strings.NewReader(expectedMetrics), "cloudevents_sent_total", "grpc_server_registered_source_clients", "grpc_server_message_received_total", "grpc_server_processed_total", "cloudevents_sent_total"); err != nil {
		t.Errorf("unexpected metrics: %v", err)
	}
}
