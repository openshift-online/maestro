...
var _ = Describe("Status Resync After Restart", Ordered, Label("e2e-tests-status-resync-restart"), func() {
	Context("Resource resync resource status after maestro server restarts", func() {
		...
		It("post the nginx resource with non-default service account to the maestro api", func() {
			res := helper.NewAPIResourceWithSA(agentTestOpts.consumerName, name, name, 1)
			...
			Eventually(func() error {
				deploy, err := agentTestOpts.kubeClientSet.AppsV1().Deployments("default").Get(ctx, name, metav1.GetOptions{})
				if err != nil {
					return err
				}
		...
		It("create serviceaccount for nginx deployment", func() {
			...
			err = agentTestOpts.kubeClientSet.AppsV1().Deployments("default").Delete(ctx, name, metav1.DeleteOptions{})
			Expect(err).ShouldNot(HaveOccurred())
		})
		...
		It("delete the nginx resource", func() {
			...
			Eventually(func() error {
				_, err := agentTestOpts.kubeClientSet.AppsV1().Deployments("default").Get(ctx, name, metav1.GetOptions{})
				if err != nil {
...
// 			err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Delete(ctx, "maestro-mqtt-server", metav1.DeleteOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})
...
// 			_, err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Create(ctx, mqttServerService, metav1.CreateOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})
