...
var _ = Describe("Spec Resync After Restart", Ordered, Label("e2e-tests-spec-resync-restart"), func() {
	Context("Resource resync resource spec after maestro agent restarts", func() {
		...
		It("post the nginx A resource to the maestro api", func() {
			res := helper.NewAPIResource(agentTestOpts.consumerName, deployA, 1)
		...
		It("post the nginx B resource to the maestro api", func() {
			res := helper.NewAPIResource(agentTestOpts.consumerName, deployB, 1)
			var resp *http.Response
			var err error
		...
		It("patch the nginx A resource", func() {
			newRes := helper.NewAPIResource(agentTestOpts.consumerName, deployA, 2)
		...
		It("post the nginx C resource to the maestro api", func() {
			res := helper.NewAPIResource(agentTestOpts.consumerName, deployC, 1)
			var resp *http.Response
			var err error
...
// 			err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Delete(ctx, "maestro-grpc-broker", metav1.DeleteOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})

// 		It("delete the mqtt-broker service for agent", func() {
// 			err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Delete(ctx, "maestro-mqtt-agent", metav1.DeleteOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})
...
// 			deploy, err := serverTestOpts.kubeClientSet.AppsV1().Deployments(serverTestOpts.serverNamespace).Get(ctx, "maestro-mqtt", metav1.GetOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 			mqttReplicas = int(*deploy.Spec.Replicas)
// 			deploy, err = serverTestOpts.kubeClientSet.AppsV1().Deployments(serverTestOpts.serverNamespace).Patch(ctx, "maestro-mqtt", types.MergePatchType, []byte(`{"spec":{"replicas":0}}`), metav1.PatchOptions{
// 				FieldManager: "serverTestOpts.kubeClientSet",
// 			})
// 			Expect(err).ShouldNot(HaveOccurred())
// 			Expect(*deploy.Spec.Replicas).To(Equal(int32(0)))
...
// 			deploy, err = serverTestOpts.kubeClientSet.AppsV1().Deployments(serverTestOpts.serverNamespace).Patch(ctx, "maestro-mqtt", types.MergePatchType, []byte(fmt.Sprintf(`{"spec":{"replicas":%d}}`, mqttReplicas)), metav1.PatchOptions{
// 				FieldManager: "serverTestOpts.kubeClientSet",
// 			})
// 			Expect(err).ShouldNot(HaveOccurred())
...
// 			_, err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Create(ctx, mqttAgentService, metav1.CreateOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})
...
// 				},
// 			}
// 			_, err := serverTestOpts.kubeClientSet.CoreV1().Services(serverTestOpts.serverNamespace).Create(ctx, grpcBrokerService, metav1.CreateOptions{})
// 			Expect(err).ShouldNot(HaveOccurred())
// 		})
