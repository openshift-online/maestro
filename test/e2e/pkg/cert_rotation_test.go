...
var _ = Describe("Certificate Rotation", Ordered, Label("e2e-tests-cert-rotation"), func() {
	Context("Certificate Rotation Tests", func() {
		...
		It("should rotate client certificate with short expiration for maestro agent", func() {
			err := rotateCertificates(ctx, 5*time.Second)
			Expect(err).ShouldNot(HaveOccurred())

			By("waiting for certificate expiration...")
			time.Sleep(20 * time.Second)
		})

		It("should verify agent reconnects with new certificates", func() {
		...
		It("should rotate client certificate with long expiration for maestro agent", func() {
			err := rotateCertificates(ctx, 3600*time.Second)
			Expect(err).ShouldNot(HaveOccurred())
		...
		It("should verify agent reconnects with new certificates", func() {
			By("updating the deployment to 3 replicas to trigger agent communication")
			work, err := sourceWorkClient.ManifestWorks(agentTestOpts.consumerName).Get(ctx, workName, metav1.GetOptions{})
			Expect(err).ShouldNot(HaveOccurred())

			newWork := work.DeepCopy()
			newWork.Spec.Workload.Manifests = []workv1.Manifest{helper.NewManifest(deployName, "default", 3)}
			patchData, err := grpcsource.ToWorkPatch(work, newWork)
			Expect(err).ShouldNot(HaveOccurred())
			...
			By("verifying the deployment is updated to 3 replicas on the agent cluster")
			Eventually(func() error {
				deployment, err := agentTestOpts.kubeClientSet.AppsV1().Deployments("default").Get(ctx, deployName, metav1.GetOptions{})
				if err != nil {
					return err
				...
				if deployment.Spec.Replicas != nil && *deployment.Spec.Replicas != 3 {
					return fmt.Errorf("expected 3 replicas, got %d", *deployment.Spec.Replicas)
				}
				return nil
			}, 1*time.Minute, 1*time.Second).ShouldNot(HaveOccurred())
		})
...
func rotateCertificates(ctx context.Context, duration time.Duration) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get maestro-mqtt-ca secret: %w", err)
	}

	mqttCACertPEM := mqttCASecret.Data["ca.crt"]
	mqttCAKeyPEM := mqttCASecret.Data["ca.key"]
	if len(mqttCACertPEM) == 0 {
		return fmt.Errorf("ca.crt not found in maestro-mqtt-ca secret")
	}
	if len(mqttCAKeyPEM) == 0 {
		return fmt.Errorf("ca.key not found in maestro-mqtt-ca secret")
	}

	mqttCACert, err := parseCertificate(mqttCACertPEM)
	if err != nil {
		return fmt.Errorf("failed to parse MQTT CA certificate: %w", err)
	}
	mqttCAKey, err := parsePrivateKey(mqttCAKeyPEM)
	if err != nil {
		return fmt.Errorf("failed to parse MQTT CA key: %w", err)
	}

	newMQTTClientCertPEM, newMQTTClientKeyPEM, err := signClientCertificate(mqttCACert, mqttCAKey, duration)
	if err != nil {
		return fmt.Errorf("failed to sign MQTT client certificate: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get maestro-agent-certs secret: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to update maestro-agent-certs secret: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get maestro-grpc-broker-ca secret: %w", err)
	}
	...
	if len(gRPCBrokerCACertPEM) == 0 {
		return fmt.Errorf("ca.crt not found in maestro-grpc-broker-ca secret")
	}
	if len(gRPCBrokerCAKeyPEM) == 0 {
		return fmt.Errorf("ca.key not found in maestro-grpc-broker-ca secret")
	}

	gRPCBrokerCACert, err := parseCertificate(gRPCBrokerCACertPEM)
	if err != nil {
		return fmt.Errorf("failed to parse gRPC broker CA certificate: %w", err)
	}
	gRPCBrokerCAKey, err := parsePrivateKey(gRPCBrokerCAKeyPEM)
	if err != nil {
		return fmt.Errorf("failed to parse gRPC broker CA key: %w", err)
	}

	newGRPCClientCertPEM, newGRPCClientKeyPEM, err := signClientCertificate(gRPCBrokerCACert, gRPCBrokerCAKey, duration)
	if err != nil {
		return fmt.Errorf("failed to sign gRPC client certificate: %w", err)
	}

	grpcSecret, err := agentTestOpts.kubeClientSet.CoreV1().Secrets(agentTestOpts.agentNamespace).Get(ctx, "maestro-grpc-broker-cert", metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("failed to get maestro-grpc-broker-cert secret: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to update maestro-grpc-broker-cert secret: %w", err)
	}
